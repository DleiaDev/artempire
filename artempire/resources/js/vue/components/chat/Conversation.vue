  ...
  <div id="second-screen">
    ...
    <div id="conversation" v-if="messages.length">
      ...
      <div class="header">
        ...
        <div class="right">
          <a :href="`/user/${contact.username}`" class="image background" :style="`background-image: url(${contact.profile})`"></a>
          <a :href="`/user/${contact.username}`" class="username">
            {{ contact.fullName }}
          </a>
        </div>
      </div>
    ...
    <div id="welcome" v-else>
      ...
      <p>Click on a contact to start chatting. If you want to add a contact, message the contact on profile page.</p>
    </div>
...
export default {
  ...
  methods: {
    ...
    sendMessage() {
      ...
      var messagesLength = this.messages.push({
        id: null,
        from: this.$user.id,
        to: this.contact.id,
        text: this.text,
